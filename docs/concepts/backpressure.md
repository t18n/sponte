...
- **`max_phase_rounds`** — counted agent phases (plan/implement/improve/verify-like steps) in a single task run/resume slice. When the limit is hit and the checklist still has pending items, the task moves to **`review-required`** and the current loop stops **for that task** so another task can run.
